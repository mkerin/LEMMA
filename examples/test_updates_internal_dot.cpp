...
int main() {
	...
	while(read()){
		...
		double c1_b = std::log(lam_b / (1.0 - lam_b) + eps) - std::log(sigma_b * sigma);
		double c1_g = std::log(lam_g / (1.0 - lam_g) + eps) - std::log(sigma_g * sigma);

		// Calling matrix * vector multiplication
		std::cout << "Testing updateAlphaMu method" << std::endl;
		auto now = std::chrono::system_clock::now();
		Eigen::VectorXd X_col_j(N);
		double rr_k, ff_k;
		for(std::uint32_t kk = 0; kk < P; kk++){
			...
			// mu(kk) = s_sq * (Hty - X.dot_with_jth_col(Hr, kk) + dHtH * rr_k) / sigma;


			// Update alpha (eq 10)  TODO: check syntax / i_  / sigmoid here!
			if (kk < P){
				ff_k = c1_b + std::log(s_sq) / 2.0;
				ff_k += mu(kk) * mu(kk) / s_sq / 2.0;
			} else {
				ff_k = c1_g + std::log(s_sq) / 2.0;
				ff_k += mu(kk) * mu(kk) / s_sq / 2.0;
			}
			...
			// X.col(kk, X_col_j);
			// new_Hr = Hr + (alpha(kk)*mu(kk) - rr_k) * (X_col_j);
