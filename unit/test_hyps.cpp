...
TEST_CASE("Hyps"){
	...
	SECTION("derived initial hyps"){
		...
		SECTION("Main effects, gaussian prior"){
			p.hyps_grid_file = "data/io_test/hyps_caseA.txt";
			...
			CHECK(all_vp.size() > 0);
			CHECK(VB.n_var == 67);
			CHECK(all_vp[0].betas->get_hyps_var() == Approx(0.0029850746));
			CHECK(all_vp[0].pve(0) == Approx(0.2));
		}

		SECTION("Joint effects, gaussian prior"){
			p.env_file = "data/io_test/n50_p100_env.txt";
			p.hyps_grid_file = "data/io_test/hyps_caseB.txt";
			p.beta_prior = "Gaussian";
			p.gamma_prior = "Gaussian";
			...
			CHECK(all_vp[0].betas->get_hyps_var() == Approx(0.0029850746));
			CHECK(all_vp[0].pve(0) == Approx(0.2));
			CHECK(all_vp[0].pve(1) == Approx(0.1));
		}
