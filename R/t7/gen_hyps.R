...
df = subset(df, h_g == h_b)
df = subset(df, logit10_lambda_b ==  logit10_lambda_g)

df$lambda_b = sigmoid10(df$logit10_lambda_b)
df$lambda_g = sigmoid10(df$logit10_lambda_g)
df$sigma_b  = pve2sigma.singleVar(df$h_b, s_x, s_z, df$lambda_b, df$lambda_g)
df$sigma_g  = pve2sigma.singleVar(df$h_b, s_x, s_z, df$lambda_b, df$lambda_g)
