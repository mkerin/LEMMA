...
bool optimNelderMead(Eigen::Ref<Eigen::VectorXd> init_out_vals,
					 std::function<double(Eigen::VectorXd vals_inp, void *grad_out)> opt_objfn, parameters p) {
	bool success = false;

	const size_t n_vals = init_out_vals.rows();

	//
	// NM settings
	const long conv_failure_switch = 0;
	const long iter_max = 2000;
	...
	while (err > err_tol && iter < iter_max) {
		...
		// step 1
		// sort from low (best) to high (worst) values
...
// arma::vec centroid = arma::trans(arma::sum(simplex_points.rows(0,n_vals-1),0)) / static_cast<double>(n_vals);
	...
	std::cout << "Nelder-Mead: search completed." << std::endl;
