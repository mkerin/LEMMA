...
class Data
{
	...
	void standardise_non_genetic_data(){
		...
		if(n_covar > 0){
			EigenUtils::center_matrix(W);
			EigenUtils::scale_matrix_and_remove_constant_cols( W, n_covar, covar_names );
		}
		if(n_env > 0){
		...
		*/
		...
		// TODO: Check columns for zero variance
	...
	void read_covar( ){
		...
		EigenUtils::read_matrix(params.covar_file, n_samples, W, covar_names, missing_covars);
		n_covar = W.cols();
		W_reduced = false;
	}
	...
	void read_environment_weights( ){
		...
		EigenUtils::read_matrix(params.env_weights_file, E_weights, col_names);
