...
void parse_arguments(parameters &p, int argc, char **argv) {
	...
	for(i = 0; i < argc; i++) {
		if(*argv[i] == '-') {
			...
			if(strcmp(in_str, "--env_momentum_coeff") == 0) {
				p.env_momentum_coeff = std::stod(argv[i + 1]);
				p.mode_env_momentum = true;
				std::cout << "Adding momentum to env weights update with parameter: " << p.env_momentum_coeff << std::endl;
				i += 1;
			}
			...
			if(strcmp(in_str, "--mode_vb_accelerated") == 0) {
				p.mode_vb = true;
				p.mode_vb_accelerated = true;
			}

			if(strcmp(in_str, "--delay_acceleration") == 0) {
				p.mode_vb = true;
				p.mode_vb_accelerated = true;
				p.mode_vb_accelerated_delay = std::stol(argv[i + 1]);
				i += 1;
			}

			if(strcmp(in_str, "--DEBUG_mode_vb_accelerated") == 0) {
				p.mode_vb = true;
				p.mode_vb_accelerated = true;
				p.leave_theta_constant = true;
			}
