...
GaussianVec& operator = (const GaussianVec &dist){
	nn = dist.nn;
	nats = dist.nats;
	return *this;
}
...
double kl_div(const double& sigma) const {
...
MoGaussianVec& operator = (const MoGaussianVec &dist){
	nn = dist.nn;
	mix = dist.mix;
	slab_nats = dist.slab_nats;
	spike_nats = dist.spike_nats;
	return *this;
}
/*** Basic properties ***/
Eigen::VectorXd mean() const {
...
double kl_div(const double& lambda, const double& sigma_slab, const double& sigma_spike) const {
	Eigen::ArrayXd res;
